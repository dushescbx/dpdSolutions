 function inOutTable = GainVsInPowAndTableGenDPD(InputWaveformdBm,...
    OutputWaveformdBm, OutputWaveform, InputWaveform, figEn)
if figEn
    figure;
...
for p = minIdx+1:length(edges)-1
    inOutTable(p-minIdx,1) = mean(OutputWaveformdBm(idx == p));   % Average input power for current bin
...
% output power
inOutTable(:, 2) = NaNReplace(inOutTable(:, 1), "OutputPower", figEn);
% phase
