function evm = synch_compare(data_train, data_ref, mat_or_real, raw_or_iq)
%% уравниваем мощность
if (mat_or_real == 0 && raw_or_iq == 1)
    data_train = data_train(1:length(data_ref)/10);
    data_ref = data_ref(6:length(data_train)+5);
elseif (mat_or_real == 0 && raw_or_iq == 0)
    data_train = data_train(1:length(data_ref)/10);
    data_ref = data_ref(1:length(data_train));
else
    data_train = data_train(1:length(data_ref)/10);
    data_ref = data_ref(1:length(data_train));
end
