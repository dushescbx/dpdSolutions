...
%
% *************************************************************************
function Convert_Mat2Wv_function(filename, ip)
% input file
%%ВЫБОР ВХОДНЫХ ФАЙЛОВ

% [filename, pname] = uigetfile('*.mat','MultiSelect','on'); %вызов диалогового окна для выбора файлов
% filename = fullfile(pname,filename); %fname хранит полный путь в файлам. Массив 1хN, N - кол-во выбранных файлов
...
VisaDevStr = ['TCPIP0::' ip '::inst0::INSTR'];

[status, InstrObject] = rs_connect( 'visa', 'ni', VisaDevStr );
% if( ~status )
...
[status] = rs_generate_wave (InstrObject, IQInfo, playback, save_local);
if (status<1)
    disp ('*** Failed to generate waveform');
    clear;
    return;
end
