...
for i=1:1:length(input_ar.d)
    ...
    load([direct '\amplifier_usil_mal\' num2str(APSK_ar(f)) 'APSK_ref_symb_timeseries.mat']);
    ...
    data_in=[data_in; data_in_ADD(18:length(data_in_ADD))];
    
    %% ÇÀÃĞÓÆÀÅÌ ñèìâîëû Ñ×ÈÒÀÍÍÛÅ ÈÇ ÀÍÀËÈÇÀÒÎĞÀ
    load([direct '\ampl_scripts\output_input_signals\_' num2str(APSK_ar(f)) 'APSK'  num2str(input_ar.d(i)) 'dbm_DPD_on=False_pa_on=True_trace.dat.mat'])
    ...
    data_out_ADD=scale_power(data_out_ADD(18:length(data_in_ADD)),output_ar.d,i);
...
for i=1:1:length(current_dbm_ar_for_ampl_char)
    ...
    simin.data=scale_power(simin.data(18:end),current_dbm_ar_for_ampl_char,i);
