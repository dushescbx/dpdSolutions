    ...
    for ka=1:1:length(current_dbm_ar)
        for dpd_on=0:1:1
            ...
            hfin = fopen(['C:\Users\Konstantinov_PA\Desktop\amplifier\ampl_scripts\output_input_signals\in_pow_out_pow_evm_array_' ...
                num2str(APSK_ar(l)) 'APSK_PA_ON=' PA_ON_text '_dpd_on=' dpd_or_not_dpd_text 'av_input_power=' num2str(current_dbm_ar(ka)) '.txt'],'r');
            i=1;
            while ~feof(hfin)
                tline = fgetl(hfin);
        ...
        legend('wo dpd',['with dpd_av_pow=',num2str(current_dbm_ar(ka))]);
        figure;
