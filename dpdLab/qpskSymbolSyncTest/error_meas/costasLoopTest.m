...
for i = 1 : param.test.iterN
    ...
    signal.yqDist =...
        timing_phase_freq_offset(signal.yq, 0*delta_f(i), ...
    ...
    %% freq shift
    signal.yqDistFreq =... %!!!!!!!!!!!!!!!!!! *0
        phase_freq_timing_offset(signal.yqDist, -0*offsetFreqCoarse(i), ...
        0, 0);
    %                 signal.yqDistFreq =...
    ...
    signal.rx_symbolDist =... %!!!!!!!!!!!!!!!!!! *0
        phase_freq_timing_offset(signal.rx_symbol, delta_f(i), ... %5e-5
        0, 0);
        [ signal.dOutShiftFinderTest, signal.xShiftedTest, delay, phase ] = ...
        shiftFinder( signal.rx_symbolDist, ...
        signal.x, 1);
        [angle_shiftTestSymb(i), ...
        freq_shiftTestSymbFFT(i), freq_shiftTestSymb(i)] =...
        freq_phase_correction(signal.dOutShiftFinderTest, signal.xShiftedTest);
%     
    [~, ... %signal.rx_symbolFreq_phase_corrected
        freqEst(i)] = costasLoop(signal.rx_symbolDist,...
        param.costasLoop, param.M, 1);
        signal.rx_symbolFreq_phase_corrected =... %!!!!!!!!!!!!!!!!!! *0
        phase_freq_timing_offset(signal.rx_symbolDist, -delta_f(i), ... %1*freqEst(i)
        0, 0);
    %% shift finder
